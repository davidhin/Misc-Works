...
void partialSort(int* a, int n, int k)
{
	int* temp = new int[k], counter = 0;
	for (int i = n - 1; i > 0; i--)
	{
		swap(a, 0, i);
...
{
	int arrSize, *array, k;
	std::fstream infile("data-15a.txt");
	infile >> arrSize;
...
/**/	std::cout << "Original Array: "; print(array, arrSize);
	minHeap(array, arrSize);
/**/	std::cout << "MnHp: "; print(array, arrSize);
	partialSort(array, arrSize, k);
